...
from yolken_test_13a import YolkenTest13a, AsyncYolkenTest13a, DefaultAioHttpClient
from yolken_test_13a._utils import is_dict
...
logging.getLogger("yolken_test_13a").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[YolkenTest13a]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with YolkenTest13a(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncYolkenTest13a]:
